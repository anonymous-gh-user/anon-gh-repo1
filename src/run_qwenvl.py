...
from src.models.qwen import QwenGenerator
...
class Qwen2VLExperiment:
    ...
    def setup_model(self):
        ...
        self.model = QwenGenerator(device=self.config.device, prompt_mode=self.config.qwen.prompt_mode)
    ...
    def run_eval_epoch(self, loader, desc="eval"):
        ...
        col_names = 'img_name,birads,label,concepts,llm_output,report_summary,parsed_concepts'
        ...
        for eval_iter, batch in enumerate(tqdm(loader, desc=desc)):
            ...
            if "roi_id" in batch.keys():
                metadata_for_qwen["roi_id"] = batch["roi_id"][0]

            if self.config.qwen.include_mask:
                metadata_for_qwen["mask"] = mask
            if self.config.qwen.include_bbox:
                pass

            # run the model
            with torch.cuda.amp.autocast(enabled=self.config.use_amp):
                # forward pass
                text_output = self.model.generate_clinical_report(x, metadata_for_qwen)[0]
                data = {
                    'img_name': x,
                    'label': [y.item()],
                    'concepts': c.tolist(),
        ...
        llm_out_df = pd.DataFrame(columns=col_names.split(','))
        for item in llm_outputs:
            #llm_out_df = llm_out_df.append(item, ignore_index=True)
            llm_out_df = pd.concat([llm_out_df, pd.DataFrame(item)], ignore_index=True)
        # save the dataframe to a csv file
        llm_out_df.to_csv(f'./{self.config.qwen.prompt_mode}_{desc}_{self.config.data.dataset}.csv', index=False)
