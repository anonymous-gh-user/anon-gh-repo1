...
class BiomedCLIPExperiment:
    ...
    def setup_model(self):
        ...
        self.model = BiomedCLIP(self.config)
        ...
        # concept detection classifier (multi-label classification)
    ...
    def run_train_epoch(self, loader, desc="train"):
        ...
        for train_iter, batch in enumerate(tqdm(loader, desc=desc)):
            ...
            with torch.cuda.amp.autocast(enabled=self.config.use_amp):
                ...
                y_pred = self.model.get_cancer_pred(img_feats)
    ...
    def run_eval_epoch(self, loader, desc="eval"):
        ...
        if self.config.data.dataset != 'BUSBRA':
            ...
            clf_metrics_probe = compute_classification_metrics(all_labels, y_pred_probe, desc=desc)
        ...
        y_pred = self.model.get_cancer_pred(torch.tensor(all_img_feats, device=self.config.device))
