...
from src.models.clip import CLIPRN50, CLIPViT, CLIPViTL, SigLIP
...
class ReasoningExperiment:
    ...
    def setup(self):
        ...
        #self.setup_optimizer() # NO TRAINING
        #if self.state is not None:
    ...
    def setup_model(self):
        ...
        self.model = ReasoningModel(device=self.config.device)
    ...
    def run_eval_epoch(self, loader, desc="eval"):
        ...
        for eval_iter, batch in enumerate(tqdm(loader, desc=desc)):
            ...
            print(f"c_pred shape: {c_pred.shape} c_pred {c_pred}")
        ...
        llm_out_df.to_csv(f'./{self.config.llama.prompt_mode}_reasoning_{desc}_{self.config.data.dataset}_fold{self.config.data.fold}.csv', index=False)
