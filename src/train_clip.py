...
from src.models.clip import CLIPRN50, CLIPViT, CLIPViTL
...
class CLIPExperiment:
    ...
    def setup_model(self):
        ...
        if self.config.clip.backbone == 'resnet':
            self.model = CLIPRN50(self.config)
        elif self.config.clip.backbone == 'vit':
            self.model = CLIPViT(self.config)
        else:
            self.model = CLIPViTL(self.config)
